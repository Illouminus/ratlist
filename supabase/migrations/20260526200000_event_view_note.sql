-- ============================================================================
-- Include item.note in get_event_view payload.
-- ============================================================================
-- The viewer-facing event-landing/event-detail screens want to surface the
-- owner's personal note on each curated item. Friend list and public share
-- already see `note` (it's on the items table and `public_item` composite);
-- the gap was the events RPC, which only returned title/cover_url/url/
-- price_text/maker/priority.
--
-- Items payload is built inline as `jsonb_build_object(...)` — no composite
-- type to alter, just a CREATE OR REPLACE on the function. Body is identical
-- to the prior version except for the new `'note', i.note` row.
-- ============================================================================
